...
class TitleBar(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 布局
        layout = QHBoxLayout(self, spacing=0) # 水平布局
        layout.setContentsMargins(0, 0, 0, 0)

        # 窗口图标
        self.iconLabel = QLabel(self)
        self.iconLabel.setMargin(5)  # 设置图标logo的margin
        layout.addWidget(self.iconLabel)

        # 窗口标题
        self.titleLabel = QLabel(self)
        self.titleLabel.setMargin(5)
        self.setTitleStyle()  # 窗口标题样式
...
if __name__ == '__main__':
    ...
    with open("./qss/titlestyle.qss", "r", encoding="UTF-8") as f:
        style = f.read()
    f.close()

    app.setStyleSheet(style)
    mainWnd = FramelessWindow()
    mainWnd.setWindowTitle('标哥深度学习常见算法')
    mainWnd.setWindowIcon(QIcon('./img/logo.png'))
